// WindowsProject4-Client2.cpp : Defines the entry point for the application.
//

#include "framework.h"
#include "WindowsProject4-Client2.h"
                     ...
                     _In_ int       nCmdShow)
{
    ...
    LoadStringW(hInstance, IDC_WINDOWSPROJECT4CLIENT2, szWindowClass, MAX_LOADSTRING);
    ...
    HACCEL hAccelTable = LoadAccelerators(hInstance, MAKEINTRESOURCE(IDC_WINDOWSPROJECT4CLIENT2));
...
ATOM MyRegisterClass(HINSTANCE hInstance)
{
    ...
    wcex.hIcon          = LoadIcon(hInstance, MAKEINTRESOURCE(IDI_WINDOWSPROJECT4CLIENT2));
    wcex.hCursor        = LoadCursor(nullptr, IDC_ARROW);
    wcex.hbrBackground  = (HBRUSH)(COLOR_WINDOW+1);
    wcex.lpszMenuName   = MAKEINTRESOURCEW(IDC_WINDOWSPROJECT4CLIENT2);
...
BOOL InitInstance(HINSTANCE hInstance, int nCmdShow)
{
   ...
   HWND hWnd = CreateWindowW(szWindowClass, L"Client 2", WS_OVERLAPPEDWINDOW,
...
void SendMsg(HWND hWnd) // повідомлення від клієнта 2
{
    RECT rect;
    GetWindowRect(hWnd, &rect);
    // горизонт. товщина рамки змінного вікна, а не розмір вікна повідомлень
    int borderThickness = GetSystemMetrics(SM_CXFRAME); 
    // макс. ширина вікна, а не кількість відкритих процесів 
    int maxWndWidth = GetSystemMetrics(SM_CXMAXTRACK); 

    HDC hdc = GetDC(NULL); // Контекст для DPI

    sprintf_s(szBuf, "Frame thickness: %d \r\nWindow max width: %d\r\nDPI vertically: %lu\r\n",
        borderThickness, maxWndWidth, GetDeviceCaps(hdc, LOGPIXELSY));
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_CREATE:
        ...
        // WSAStartup дозволяє програмі або DLL вказувати потрібну версію Windows Sockets і отримувати деталі конкретної реалізації Windows Sockets
    ...
    case WSA_NETEVENT:
        ...
        if (WSAGETSELECTEVENT(lParam) == FD_READ)
        {
            ...
            {
                szBuf[rc] = '\0';
                sprintf_s(mess, "Data from server: %s\r\n ", szBuf);
                SendMessageA(hwndEdit, WM_SETTEXT, 0, (LPARAM)m_mess);
            }
        }
